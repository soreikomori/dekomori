...
class rjc:
    ...
    @staticmethod
    def toggle(guild: discord.Guild):
        """
        Toggles the rejoin checker for a specific guild.

        Parameters
        ----------
        guild : discord.Guild
            The guild object to toggle the rejoin checker for.

        Returns
        -------
        bool
            The new state of the rejoin checker.
        """
        guildId = str(guild.id)
        rjc = get_value(guildId, "rejoin_checker")
...
def get_all_bait_roles(guild: discord.Guild):
    """
    Retrieves all bait roles for a specific guild.
    ...
    -------
    list of int
        A list of role IDs representing all bait roles.
    """
    guildId = str(guild.id)
    bait_roles = get_value(guildId, "bait_roles")
    return bait_roles if bait_roles else []
